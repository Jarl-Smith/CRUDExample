...
namespace CRUDExample.Filters.ActionFilters {
    public class PersonListActionFilter : IActionFilter {
        ...
        private static readonly List<string> _searchByOptions = new List<string>() {
        ...
        private static readonly IDictionary<string, string> _searchField = new Dictionary<string, string>() {
        ...
        //在进入Controller的Action方法前，此处验证客户端发来的QueryString的合法性，并进行修正
        public void OnActionExecuting(ActionExecutingContext context) {
            ...
            if(context.ActionArguments.ContainsKey("searchBy")) {//如果QueryString参数有searchBy
                ...
                if(!string.IsNullOrEmpty(searchBy)) {//如果searchBy的值非空
                    if(!_searchByOptions.Any(temp => temp == searchBy)) {//且searchBy的值不匹配任意PersonResponse的列名
                        context.ActionArguments["searchBy"] = _searchByOptions[0];//searchBy修正为PersonName
                    }
                }
            }
        }
        ...
        public void OnActionExecuted(ActionExecutedContext context) {
            ...
            //由于OnActionExecuted的ActionExecutedContext无法直接访问客户端发来的QueryString，所以需要在OnActionExecuting中把QueryString保存在Items中，OnActionExecuted的ActionExecutedContext可以通过访问Items来间接访问QueryString，QueryString设置到ViewData中，方便View访问
            ...
            controller.ViewData["ColumnNameAndProperty"] = _searchField;
        }
    }
}
